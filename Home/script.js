...
function F_cPeek(theObject, theParm) {
	...
	//Set distances
//	alert(theObject.getClipHeight()+" " +theObject.getClipBottom())
...
//	alert(this.controller.direction+" "+width+" "+height);
...
//		alert(this.showHide +" "+ ((this.showHide == "hide") & !this.flipped));
		if ((this.showHide == "hide") & !this.flipped) d.setVisibility("hidden");
		this.flipped=false;
...
//		alert(y+" "+theRatio);
	...
	var a = (newOffset.x - this.offset.x);
//	alert(newOffset.y +" "+ this.offset.y);
	var b = (newOffset.y - this.offset.y);
//	alert(b);
	if((a != 0) || (b != 0)) {	//move
		d.offset(a, b);
	}
		    ...
		    */
				  ...
				  with(d) {
					this.object.setClipRect(new F_cRect( 
						getClipLeft()   + (newOffset.left   - this.offset.left),
						getClipTop()    + (newOffset.top    - this.offset.top),
						getClipRight()  + (newOffset.right  - this.offset.right),
						getClipBottom() + (newOffset.bottom - this.offset.bottom)			 	
						));
			 	  }
			 }
	}
...
//	alert(this.endTime+" "+ this.startTime+" "+this.duration*100);
}
...
function F_drag (x, y) {
	if(document.F_dragLayer != null) {
		for(var i=(document.F_dragLayer.length-1);i>-1;i--) {
			...
			if(d.actions['Dragged'] != null) sendMsg(d.styleID, 'Dragged', '', null);
			if (d.dragCollision != null) F_checkCollision(d, "drag");
		}
	}
}
...
function F_setImage(theParm) {
	var a = ["Image 1 (Normal)","Image 2 (Highlighted)","Image 3 (Depressed)","Image 4 (Selected)",
